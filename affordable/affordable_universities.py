...
cur.execute("""
    ...
    JOIN (
        ...
        FROM Tuition
		WHERE Tuition.tuition IS NOT NULL
        GROUP BY university_id
    ) Tuition ON Universities.id = Tuition.university_id
    ORDER BY Tuition.average_tuition ASC
    LIMIT 10;
""")
universities = dict()
for message_row in cur :
    universities[message_row[0]] = message_row[1].replace("'", "\\'")
...
fhand = open('affordable/affordable_universities.js','w')
fhand.write("affordableUniversities = [ ['Year'")
